...
class Encoder extends stream.Transform {
  ...
  constructor(options) {
    ...
    this.dateType =
      opts.dateType != null ? opts.dateType.toLowerCase() : "number";
    ...
    this.semanticTypes = [
      ...
      this._pushArray,
      Float32Array,
      this._pushFloat32Array,
      Float64Array,
      this._pushFloat64Array,
    ];
  ...
  _transform(fresh, encoding, cb) {
    console.log("<_transform> fresh", fresh);
  ...
  _pushUInt8(val) {
    console.log("<_pushUInt8> val", val);
    // 空の1byteを作る
    const b = Buffer.allocUnsafe(1);
    // 8ビット符合無し整数を0オフセットで書き込む
    b.writeUInt8(val, 0);
    console.log("<_pushUInt8> b", b);
    return this.push(b);
  }

  _pushUInt16BE(val) {
    console.log("<_pushUInt16BE> val", val);
    const b = Buffer.allocUnsafe(2);
    b.writeUInt16BE(val, 0);
    console.log("<_pushUInt16BE> b", b);
    return this.push(b);
  }
  ...
  _pushInt(obj, mt, orig) {
    console.log("<_pushInt>_pushInt");
    console.log("<_pushInt>obj", obj);
    console.log("<_pushInt>mt", mt);
    // 	The 5-bit additional information is either the integer itself (for additional information values 0 through 23) or the length of additional data.
    const m = mt << 5; // 5bitシフト
    console.log("<_pushInt>m", m);
    switch (false) {
      case !(obj < 24):
        console.log("<_pushInt>m | obj", m | obj);
        // FIXME: ここのmは不要そう -> 負の数の時に聞いてくる
        return this._pushUInt8(m | obj);
      case !(obj <= 0xff):
        // 255まで
        console.log("<_pushInt>ff obj", obj);
        console.log("<_pushInt>ffobj m", m);
        console.log("<_pushInt>ffobj NUMBYTES.ONE", NUMBYTES.ONE);
        console.log("<_pushInt>ffobj m | NUMBYTES.ONE", m | NUMBYTES.ONE);
        return this._pushUInt8(m | NUMBYTES.ONE) && this._pushUInt8(obj);
      case !(obj <= 0xffff):
        // 65535まで
        // additional information is 25 means a uint16_t
        // エンコーダーの実装はadditional informationみなくていい
        console.log("ffff obj", obj);
        console.log("<_pushInt>ff obj", obj);
        console.log("<_pushInt>ffobj m", m);
        console.log("<_pushInt>ffobj NUMBYTES.TWO", NUMBYTES.TWO);
        console.log("<_pushInt>ffobj m | NUMBYTES.TWO", m | NUMBYTES.TWO);
  ...
  _pushString(obj) {
    console.log("<_pushString> obj", obj);
    const len = Buffer.byteLength(obj, "utf8");
    console.log("<_pushString> len", len);
   ...
   */
  _pushObject(obj) {
    // switchでこっちにnullはこないはずなので不要かも
    if (!obj) {
      return this._pushNull(obj);
    }
    // json入れ子がloopしたことを検知した時に、オプションの指定次第では例外を投げれる
    if (this.detectLoops) {
    ...
    const cbor_keys = {}; // endoceしたkeyを保存する
    // 正規化すべきかのフラグ
    if (this.canonical) {
      ...
      keys.sort((a, b) => {
        ...
        const a_cbor = cbor_keys[a] || (cbor_keys[a] = 2);
        const b_cbor = cbor_keys[b] || (cbor_keys[b] = 3);
  ...
  pushAny(obj) {
    switch (typeof obj) {
      ...
      default:
        throw new Error(
          "Unknown type: " + typeof obj + ", " + (!!obj ? obj.toString() : "")
        );
    }
  }
  ...
  _encodeAll(objs) {
    const bs = new NoFilter({ highWaterMark: this.readableHighWaterMark });
    this.pipe(bs); // streamを実装しているからpipeを呼べる
    for (const o of objs) {
      this.pushAny(o); // 流れてくるstreamを次々押し込んでいく
    }
    console.log("<_encodeAll> this", this.read);
    this.end();
    return bs.read();
  }
  ...
  static encodeAsync(obj, options) {
    console.log("<encodeAsync> obj", obj);
...
console.log("================");
// console.log("================");
// console.log(Encoder.encode(256));
// console.log("================");
// console.log(Encoder.encode(-256));
